...
/*   Updated: 2022/08/04 21:52:26 by iamongeo         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	get_mandelbrot_pix_color(t_pix *pix, double dist, int iters, int *palette, int print)
{
	int	*pcols[2];
	int	colors[3];
	int	color;
...
//				pix.palette = frm->palette;
				color = get_mandelbrot_pix_color(&pix, dist, iters, frm->palette, (!y && !x));
				if ((y == y_start) && !x)
